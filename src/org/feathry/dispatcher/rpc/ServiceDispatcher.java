...
public class ServiceDispatcher
{
	...
	{
		ClassWrapper cls = new ClassWrapper(getServiceClass());
		log.debug("Invoking RPC {}", rpc);
		try
		{
			Method method = cls.getMethod(rpc.getMethodName(), rpc.getParameterTypes());
		...
		{
			if ("getClass".equals(rpc.getMethodName()) || "toString".equals(rpc.getMethodName()))
			{
				Method m = Object.class.getMethod(rpc.getMethodName(), rpc.getParameterTypes());
				return m.invoke(serviceInstance, rpc.getArguments());
			}
	...
	{
		log.debug("Method '{}' not found trying to handle it dynamically", rpc.getMethodName());
		...
		// TODO call findMethod before and after this block because of Maps and
		// DynaBeans in arguments
		...
		try
		{
			Method method = this.findMethod(newRpc, serviceClass, methods);
	...
	protected Method findMethod(RemoteProcedureCall rpc, Class<?> serviceClass, List<Method> choices)
			throws SecurityException, NoSuchMethodException
	{
		for (Method m : choices)
		{
			...
			{
				return serviceClass.getMethod(rpc.getMethodName(), types);
			}
		}
		return serviceClass.getMethod(rpc.getMethodName(), rpc.getParameterTypes());
	}
