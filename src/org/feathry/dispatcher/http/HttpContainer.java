...
public class HttpContainer implements Container
{
	 ...
	 */
	@Override
	public void handle(Request requqest, Response response)
	{
		response.set("Server", "HelloWorld/1.0 (Simple 4.0)");
		String url = requqest.getPath().getPath(0, 1).substring(1);
		Worker worker = this.getWorkerFactory(url).create(requqest, response);
